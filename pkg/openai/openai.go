...
import (
	...
	"github.com/xctf-io/xctf/pkg/gen/jsonschema"
...
func (s *Agent) Prompt(
	...
	llm, err := openai.New(openai.WithToken(s.config.APIKey), openai.WithModel("gpt-3.5-turbo-0613"))
	if err != nil {
		return "", err
	}
	...
	completion, err := llm.GenerateContent(ctx, []llms.MessageContent{
		llms.TextParts(schema.ChatMessageTypeHuman, clean),
	})
	if err != nil {
		return "", err
	}
...
func (s *Agent) Call(
	...
	completion, err := llm.GenerateContent(ctx,
		[]llms.MessageContent{
			llms.TextParts(schema.ChatMessageTypeHuman, content)},
		llms.WithFunctions(functions))
	if err != nil {
		return nil, err
	}
	if len(completion.Choices) == 0 {
		return nil, errors.New("no choice for function call")
	}
	return completion.Choices[0].FuncCall, nil
}
